...
// Adapted from the dashboard TranscriptSegment; provenance in panel/src/README.md.
import React from 'react';
const cn=(...values:any[])=>values.filter(Boolean).join(' ');
const parseUTCTimestamp=(value:string)=>new Date(/[zZ]$|[+-]\d{2}:\d{2}$/.test(value)?value:value+'Z');
const Play=({className}:any)=><span className={className}>▶</span>;
type TranscriptSegmentType=any;
type SpeakerColor={text:string};

interface TranscriptSegmentProps {
  segment: TranscriptSegmentType;
  annotatedText?: React.ReactNode;
        ...
        {!showSpeakerHeader && (
          <div className="flex items-center gap-2">
            <p className={cn("text-sm leading-snug flex-1", !segment.completed && "text-muted-foreground/70 italic")}>
              {annotations}{annotatedText ?? renderText(segment.text, searchQuery)}
            </p>
